 ...
 *   bun run scripts/release.ts --bump patch --create-release
...
function readVersionFiles(): VersionFiles {
  return {
    ...
    cargoToml: readFileSync(join(PROJECT_ROOT, "src-tauri/Cargo.toml"), "utf-8"),
    tauriConf: readFileSync(join(PROJECT_ROOT, "src-tauri/tauri.conf.json"), "utf-8"),
  };
}
...
async function createGitHubRelease(version: string): Promise<void> {
  ...
  if (!token) {
    console.error("❌ GITHUB_TOKEN or GH_TOKEN environment variable is required to create a release");
    process.exit(1);
  }
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--create-release" || arg === "--release") {
      ...
      if (type === "patch" || type === "minor" || type === "major") {
        ...
        console.error(`❌ Invalid bump type: ${type}. Must be patch, minor, or major`);
        process.exit(1);
      }
  ...
  if (bumpType) {
    ...
    console.log(`🔄 Bumping version from ${currentVersion} to ${targetVersion} (${bumpType})`);
  } else if (!targetVersion) {
    // If no version provided, just sync current versions
  ...
  if (!/^\d+\.\d+\.\d+$/.test(targetVersion)) {
    console.error(`❌ Invalid version format: ${targetVersion}. Must be semver (e.g., 0.1.0)`);
    process.exit(1);
  }
